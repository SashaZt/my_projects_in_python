...
class ConfigProcessor:
        ...
        self.service_mappings = {
            "postgres": ComponentMapping(
                ...
                custom_mappings={
                    ...
                    "db_init_db": "POSTGRES_INITDB_WALDIR",
            ...
            "client": ComponentMapping(
                name="client", sections=["client"], env_prefix="CLIENT_"
            ),
    ...
    def generate_component_config(
        ...
        try:
            ...
            if output_format == OutputFormat.ENV:
                ...
                output_path = os.path.join(output_dir, f"{component}_config.json")
                self.write_json_file(component_config, output_path)

            elif output_format == OutputFormat.DOCKER_ENV:
                env_vars = self.generate_env_variables(component_config, mapping)
                output_path = f".env.{component}"
                self.write_env_file(env_vars, output_path, component)
    ...
    def generate_main_env(self, output_file: str = ".env") -> bool:
        ...
        try:
            ...
            global_vars = [
                ...
                if k in ["PROJECT_NAME", "VERSION", "ENVIRONMENT", "TIMEZONE"]
            ]
        ...
        try:
            for service_name, mapping in self.service_mappings.items():
                if any(section in self.config for section in mapping.sections):
                    ...
                    if service_name in ["postgres", "db"]:
                        # Для PostgreSQL берем только PG_ переменные и основные POSTGRES_
                        for key, value in env_vars.items():
                            if key.startswith(("PG_", "POSTGRES_", "PGDATA")):
    ...
    def create_service_directories(self) -> bool:
        ...
        global_keys = [
            k for k in self.config.keys() if not isinstance(self.config[k], dict)
        ]
        if global_keys:
        ...
        # Секции компонентов
        component_sections = [
            k for k in self.config.keys() if isinstance(self.config[k], dict)
        ]
        if component_sections:
            ...
            for section in component_sections:
                ...
                print(f"   • {section} ({items_count} параметров)")
            print()
