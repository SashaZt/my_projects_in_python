...
#                     items = [
#                         span.parent for span in title_spans if span.parent.name == "a"
#                     ]
...
#                 )
#                 return next_button["href"], page_hrefs
        ...
        for condition in current_conditions:
            ...
            while True:
                ...
                if page_hrefs:
                    ...
                    # Сохраняем URL для отчетности
                    urls_df = pd.DataFrame(page_hrefs, columns=["href"])
                    file_name = f"urls_{brand}_{condition}_page_{current_page}.csv"
...
if __name__ == "__main__":
    ...
    # Для запуска только сбора URL:
    # urls = run_scraper(max_pages=50)

    # Для запуска только скачивания страниц товаров:
    # df = pd.read_csv(data_directory / "all_urls.csv")
    # success_count = get_product_th(df["href"].tolist(), threads=20)        save_progress(progress)

    # # Объединяем все собранные URL
    # final_df = pd.DataFrame(all_collected_urls, columns=["href"])
    # final_df = final_df.drop_duplicates()
    # final_path = data_directory / "all_urls.csv"
    # final_df.to_csv(final_path, index=False)
    # logger.info(f"Всего собрано {len(final_df)} уникальных ссылок в all_urls.csv")

    # # Возвращаем список собранных URL
    # return final_df["href"].tolist()
...
#     # Для запуска только скачивания страниц товаров:
#     # df = pd.read_csv(data_directory / "all_urls.csv")
#     # success_count = get_product_th(df["href"].tolist(), threads=20)
