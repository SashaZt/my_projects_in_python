...
try:
    ...
    os.makedirs(cookies_dir, exist_ok=True)  # Исправлено
    ...
    cookies_dir = os.path.join(work_dir, "cookies")  # Исправлено
    os.makedirs(work_dir, exist_ok=True)
    os.makedirs(cookies_dir, exist_ok=True)  # Исправлено
...
async def get_cookies(browser):
    ...
    try:
        ...
        # cookies_file = os.path.join(cookies_dir, f"cookies_all_{timestamp}.json")
        ...
        # # ИСПРАВЛЕНО: Сохранение сессии браузера
        # session_file = os.path.join(cookies_dir, "browser_session.dat")
        # await browser.cookies.save(session_file)
        # logger.info(f"Сессия браузера сохранена в {session_file}")

        # # Резервная копия
        # backup_file = os.path.join(cookies_dir, f"session_backup_{timestamp}.dat")
        # await browser.cookies.save(backup_file)
