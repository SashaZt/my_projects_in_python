...
class TikTokMonitor:
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                client = TikTokLiveClient(
                    ...
                    # proxies=None,  # Можно добавить прокси, если они у вас есть
                    # timeout_ms=15000,  # Увеличиваем таймаут до 15 секунд
                    # retry_count=3,  # Количество повторных попыток
                    # retry_delay=1000,  # Задержка между повторами (мс)
                    # fetch_room_id_from_web_api=True,  # Пытаться получить room_id через веб API
                    # fetch_room_info_on_connect=True,  # Получать информацию о комнате при подключении
                )
                ...
                # @client.on(WebsocketResponseEvent)
                # async def on_websocket_response(event):
                # logger.debug(f"Received WebSocket response for {connect_id}")
    ...
    #                 list(self.processed_gift_ids)[:5000]
    #             )
    ...
    #             event, "to_member_id", unique_id.replace("@", "")
    #         )

    #         # Используем unique_id как receiver_unique_id (это гарантированно содержит @ для имени)
    ...
    #         }
    #         # Добавляем поиск стримера перед добавлением в очередь
    #         try:
    #             # Пытаемся найти стримера любыми доступными способами
    #             streamer_data = None

    #             # 1. Сначала пытаемся найти по имени
    ...
    #             # 2. Затем пытаемся найти по user_id
    #             if not streamer_data and receiver_user_id:
    #                 streamer_data = await self.db.fetchrow(
    #                     """
    #                     SELECT s.id, s.tik_tok_user_id
    #                     FROM streamers s
    ...
    #             # 3. Если не нашли, пытаемся по любым идентификаторам
    #             if not streamer_data:
    #                 streamer_data = await self.db.fetchrow(
    #                     """
    #                     SELECT s.id, s.tik_tok_user_id
    ...
    #                         else unique_id
    #                     ),
    #                     unique_id,  # Проверяем числовой ID как user_id
    #                 )
    ...
    #                 ]
    #                 logger.info(
    #                     f"Found streamer ID {streamer_data['id']} for gift to {unique_id}"
    #                 )
    #             else:
    #                 logger.warning(
    #                     f"No streamer found for gift to {unique_id} (user_id: {receiver_user_id})"
    #                 )
    #         except Exception as e:
    ...
    # async def _process_gift(self, event, unique_id, cluster):
    #     """Обработка подарка с улучшенным поиском стримера"""
    #     try:
    #         # Используем доступные атрибуты для формирования уникального ключа
    #         current_time = int(time.time() * 1000)
    ...
    #         # Улучшенный поиск стримера
    #         try:
    #             streamer_data = None
    #             room_id = None

    #             # Получаем room_id из события или клиента, если доступен
    #             room_id = getattr(
    #                 event,
    #                 "room_id",
    ...
    #             # Приоритет поиска:
    #             # 1. Используем room_id текущего стрима
    #             if room_id:
    #                 # Ищем по room_id
    #                 streamer_data = await self.db.fetchrow(
    #                     """
    ...
    #                 # Если не нашли прямо, пробуем через маппинг
    #                 if not streamer_data:
    #                     streamer_data = await self.db.fetchrow(
    #                         """
    #                         SELECT s.id, s.tik_tok_user_id
    #                         FROM streamers s
    #                         JOIN tik_tok_users ttu ON s.tik_tok_user_id = ttu.id
    #                         JOIN room_id_mapping rm ON rm.user_id = ttu.user_id
    #                         WHERE rm.room_id = $1
    #                         """,
    #                         room_id,
    #                     )

    #             # 2. Ищем по имени получателя
    #             if not streamer_data and unique_id.startswith("@"):
    #                 streamer_data = await self.db.fetchrow(
    #                     """
    #                     SELECT s.id, s.tik_tok_user_id
    #                     FROM streamers s
    ...
    #                     "tik_tok_user_id"
    #                 ]
    #                 # Записываем используя и room_id для лучшей отладки
    #                 log_room = f"room_id: {room_id}" if room_id else ""
    #                 logger.info(
    #                     f"Found streamer ID {streamer_data['id']} for gift to {unique_id} ({log_room})"
    #                 )
    #             else:
    #                 logger.warning(
    #                     f"No streamer found for gift to {unique_id} (user_id: {receiver_user_id}, room_id: {room_id})"
    #                 )
    #         except Exception as e:
    #             logger.warning(f"Error finding streamer for gift: {e}")
    ...
    #         logger.error(f"Error processing gift: {e}", exc_info=True)
    # async def _process_gift(self, event, unique_id, cluster):
    #     """Обработка подарка с определением получателя подарка"""
    #     try:
    #         # Базовая обработка атрибутов подарка (без изменений)
    #         current_time = int(time.time() * 1000)
    #         gift = getattr(event, "gift", None)
    #         user = getattr(event, "user", getattr(event, "from_user", None))
    ...
    #         # Ключ дедупликации (без изменений)
    #         gift_id = getattr(gift, "id", current_time)
    #         diamond_count = getattr(gift, "diamond_count", 0)
    #         user_id = getattr(user, "id", "0")
    #         gift_key = f"{gift_id}_{diamond_count}_{user_id}_{unique_id}_{current_time}"
    ...
    #         # Атрибуты подарка (без изменений)
    #         gift_count = getattr(
    #             event, "repeat_count", getattr(gift, "repeat_count", 1)
    #         )
    #         gift_name = getattr(gift, "name", "Unknown Gift")
    ...
    #         # Формируем данные о подарке (без изменений)
    #         gift_data = {
    #             "user_id": str(user_id),
    ...
    #         # Поиск стримера с оптимизированными приоритетами
    #         try:
    #             streamer_data = None

    #             # 1. Самый надежный метод: по комнате текущего стрима
    #             if client_room_id:
    #                 # Ищем напрямую по room_id в streamers
    #                 streamer_data = await self.db.fetchrow(
    #                     """
    #                     SELECT s.id, s.tik_tok_user_id
    #                     FROM streamers s
    ...
    #                 # Если не нашли, ищем через маппинг
    #                 if not streamer_data:
    #                     streamer_data = await self.db.fetchrow(
    #                         """
    #                         SELECT s.id, s.tik_tok_user_id
    #                         FROM streamers s
    #                         JOIN room_id_mapping rm ON s.tik_tok_user_id = rm.tik_tok_user_id
    #                         WHERE rm.room_id = $1
    #                         """,
    #                         client_room_id,
    #                     )
    ...
    #                 if receiver_user_id:
    #                     log_info.append(f"user_id: {receiver_user_id}")
    ...
    #             else:
    #                 logger.warning(
    #                     f"No streamer found for gift to {unique_id} (user_id: {receiver_user_id}, room_id: {client_room_id})"
    #                 )

    #         except Exception as e:
    #             logger.warning(f"Error finding streamer for gift: {e}")

    #         # Добавляем подарок в очередь (без изменений)
    #         logger.debug(
    #             f"Processing gift: {gift_name} x{gift_count} ({diamond_count} diamonds) from {user_unique_id}"
    #         )
    ...
    async def _process_gift(self, event, unique_id, cluster):
        ...
        try:
            ...
            try:
                ...
                # 1. Поиск через room_id текущего стрима
                if client_room_id:
                    # Ищем через room_id_mapping, чтобы получить правильный tik_tok_user_id
                    ...
                    if mapping_data:
                        # Нашли правильную связь, теперь получаем данные стримера
                        streamer_data = await self.db.fetchrow(
                            """
                            SELECT s.id, s.tik_tok_user_id
                            FROM streamers s
                        ...
                        # Устанавливаем правильный user_id для receiver_unique_id
                        actual_user_id = mapping_data["user_id"]
                        logger.info(
                            f"Found user_id {actual_user_id} for room_id {client_room_id} (@{mapping_data['name']})"
                        )
                    else:
                ...
                if not streamer_data and unique_id and unique_id.isdigit():
                    # Ищем по числовому идентификатору в room_id
                    streamer_data = await self.db.fetchrow(
                        """
                        SELECT s.id, s.tik_tok_user_id, ttu.user_id
                        FROM streamers s
                    ...
                    if streamer_data:
                        actual_user_id = streamer_data["user_id"]
                        logger.info(
                            f"Found streamer by numeric unique_id={unique_id} as room_id"
                        )
                # 2. Поиск по username
                if not streamer_data and unique_id.startswith("@"):
                    streamer_data = await self.db.fetchrow(
    ...
    #     """Обновление связей между подарками и стримерами для существующих записей"""
    #     try:
    #         # Сначала синхронизируем идентификаторы
    #         await self.sync_tiktok_ids()
    #         # Сначала попробуем обновить по receiver_unique_id, если это имя пользователя с @
    #         query1 = """
    #         UPDATE gifts g
    #         SET
    #             streamer_id = s.id,
    ...
    #         # Затем попробуем обновить по числовому идентификатору
    #         query2 = """
    #         UPDATE gifts g
    #         SET
    #             streamer_id = s.id,
    ...
    #         FROM streamers s
    #         JOIN tik_tok_users ttu ON s.tik_tok_user_id = ttu.id
    #         WHERE (g.receiver_unique_id = ttu.user_id OR g.receiver_unique_id = ttu.tiktok_id)
    ...
    #         logger.info(f"Fixed auto-generated usernames: {result3}")
    #         return True
    #     except Exception as e:
    ...
    #         JOIN tik_tok_users ttu ON s.tik_tok_user_id = ttu.id
    #         JOIN room_id_mapping rm ON rm.user_id = ttu.user_id
    #         WHERE CAST(g.receiver_user_id AS BIGINT) = rm.room_id
    #         AND g.streamer_id IS NULL;
    #         """
    ...
    #         # Обновляем по room_id с использованием маппинга
    #         # Исправленный запрос - удалена ссылка на rm.user_id, используем ttu.id вместо этого
    #         query4 = """
    #         UPDATE gifts g
    #         SET
    #             streamer_id = s.id,
    ...
    #         FROM streamers s
    #         JOIN tik_tok_users ttu ON s.tik_tok_user_id = ttu.id
    #         JOIN room_id_mapping rm ON rm.tik_tok_user_id = ttu.id
    #         WHERE CAST(g.receiver_user_id AS BIGINT) = rm.room_id
    #         AND g.streamer_id IS NULL;
    #         """
    #         result4 = await self.db.execute(query4)
