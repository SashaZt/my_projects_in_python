...
class Database:
    ...
    #     """Обновление данных стримера"""
    #     try:
    #         # Получаем связанный tik_tok_user_id и его текущий user_id
    #         tik_tok_data = await self.fetchrow(
    #             "SELECT ttu.id, ttu.user_id FROM streamers s JOIN tik_tok_users ttu ON s.tik_tok_user_id = ttu.id WHERE s.id = $1",
    #             streamer_id,
    #         )
    ...
    #         tik_tok_user_id = tik_tok_data["id"]
    #         current_user_id = tik_tok_data["user_id"]
    ...
    #         # 1. user_id еще не установлен или
    #         # 2. user_id равен текущему user_id, который мы пытаемся установить
    #         tik_tok_query = "UPDATE tik_tok_users SET last_seen = NOW()"
    #         params = []

    #         if user_id and (current_user_id is None or current_user_id == user_id):
    #             tik_tok_query += f", user_id = ${len(params) + 1}"
    #             params.append(user_id)
    ...
    #         logger.info(f"Updated streamer ID {streamer_id} with room_id={room_id_int}")
    #     except Exception as e:
    #         logger.error(f"Error updating streamer data: {e}")
    # async def update_streamer(self, streamer_id, room_id, user_id=None, tiktok_id=None):
    #     """Обновление данных стримера с защитой user_id"""
    #     try:
    #         # Получаем связанный tik_tok_user_id и его данные
    #         tik_tok_user = await self.fetchrow(
    #             """
    #             SELECT ttu.id, ttu.user_id, ttu.name
    #             FROM streamers s
    ...
    #         if not tik_tok_user:
    #             logger.error(f"No tik_tok_user found for streamer {streamer_id}")
    #             return False
    ...
    #         # Сохраняем связь между room_id и user_id, если они оба присутствуют
    #         if room_id_int > 0 and user_id:
    #             # Добавляем запись в таблицу room_id_mapping, если ее еще нет
    #             await self.execute(
    #                 """
    #                 INSERT INTO room_id_mapping (room_id, user_id, tik_tok_user_id, last_updated)
    #                 VALUES ($1, $2, $3, NOW())
    #                 ON CONFLICT (room_id) DO UPDATE
    #                 SET user_id = $2, last_updated = NOW()
    #                 """,
    #                 room_id_int,
    #                 user_id,
    #                 tik_tok_user_id,
    ...
    #             logger.info(f"Mapped room_id {room_id_int} to user_id {user_id}")

    #         # Обновляем tik_tok_users только last_seen
    #         await self.execute(
    #             "UPDATE tik_tok_users SET last_seen = NOW() WHERE id = $1",
    #             tik_tok_user_id,
    #         )

    #         # Обновляем tiktok_id только если он не был установлен или отличается
    #         if tiktok_id:
    #             await self.execute(
    #                 """
    #                 UPDATE tik_tok_users
    #                 SET tiktok_id = $1
    #                 WHERE id = $2 AND (tiktok_id IS NULL OR tiktok_id != $1)
    #                 """,
    #                 tiktok_id,
    #                 tik_tok_user_id,
    #             )
    ...
    #     try:
    #         # Проверяем, существует ли пользователь с таким именем или user_id
    #         query = "SELECT id, user_id FROM tik_tok_users WHERE "
    #         params = []
    ...
    #             # Если предоставлены дополнительные данные, обновляем их, НО НЕ user_id если он уже установлен!
