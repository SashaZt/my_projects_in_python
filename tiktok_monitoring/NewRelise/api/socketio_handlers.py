...
async def join_streamers_room(sid):
    ...
    logger.info(f"Клиент {sid} подключился к комнате стримеров")
...
async def send_streamers_update(sid):
    ...
    try:
        ...
        logger.error(f"Ошибка при отправке обновления о стримерах: {e}")
...
async def send_streamers_updates(sid):
    ...
    try:
        ...
        logger.error(f"Ошибка при отправке обновлений о стримерах: {e}")
