...
def get_url(url):
    ...
    proxies = [
        line.strip()
        for line in open("/home/parsing/1000 ip.txt", "r", encoding="utf-8")
    ]
...
def get_info(url, filename, links_counter_file, thread_id, lock):
    print(
        f'{datetime.datetime.now().strftime("%H:%M:%S")} - Обрабатывается категория {url} | (Поток -> {thread_id})'
    )
    print(
        f'{datetime.datetime.now().strftime("%H:%M:%S")} - Получение следующей страницы | (Поток -> {thread_id})'
    )
    ...
    if next_page_element and "href" in next_page_element.attrs:
        ...
        print(
            f'{datetime.datetime.now().strftime("%H:%M:%S")} - Следующая страница - {next_page_link} | (Поток -> {thread_id})'
        )
    ...
    for link in links:
        print(
            f'{datetime.datetime.now().strftime("%H:%M:%S")} - Обрабатывается объявление {link}'
        )
        ...
        with lock:
            ...
            if relative_box:
                ...
                if script_tag:
                    ...
                    match = re.search(
                        r"offerDetails:\s*({.*?})\s*,", script_content, re.DOTALL
                    )
                    if match:
                        offer_details_str = match.group(1)
                        location_match = re.search(
                            r"location:\s*'([^']+)'", offer_details_str
                        )
                        if location_match:
                            location_soup = BeautifulSoup(
                                location_match.group(1), "html.parser"
                            )
                            location = ", ".join(
                                filter(
                                    None,
                                    [
                                        region.strip()
                                        for region in location_soup.get_text().split(
                                            ","
                                        )
                                    ],
                                )
                        ...
                        phone_match = re.search(
                            r"data-full-phone-number=\"([^\"]+)\"", offer_details_str
                        )
                        if phone_match:
                            phone_numbers.add(
                                phone_match.group(1)
            ...
            if not location:
                h2_element = soup.find(
                    "h2", class_="_5cxNyc undefined", attrs={"data-v-54164881": True}
                )
                if h2_element:
                    location_parts = [
                        span.get_text(strip=True)
                        for span in h2_element.find_all("span")
                    ]
            ...
            if not location:
                ...
                if location_span:
                    location_parts = location_span.find_all(
                        "span", class_="offerLocation__region"
                    )
                    location = " ".join(
                        part.get_text(strip=True) for part in location_parts
                    )
            ...
            script_tag = soup.find(
                "script", type="application/json", attrs={"data-ssr": "true"}
            )
            if script_tag:
                try:
                    json_data = json.loads(script_tag.string)
            ...
            for element in elements:
                span = element.find("span", string="Data dodania") or element.find(
                    "span", string="Aktualizacja"
                )
                if span:
            ...
            if not time_posted:
                ...
                def extract_time(date_text):
                    ...
                    if date_text == "dziś":
                        ...
                        "tydzień temu" in date_text or "ponad tydzień temu" in date_text
                    ):
                        return current_time - datetime.timedelta(days=7)
                    elif (
                        "dwa tygodnie temu" in date_text
                        or "ponad dwa tygodnie temu" in date_text
                    ):
                        return current_time - datetime.timedelta(days=14)
                    elif (
                        "miesiąc temu" in date_text or "ponad miesiąc temu" in date_text
                    ):
                        return current_time - relativedelta(months=1)
                    elif (
                        "pół roku temu" in date_text
                        or "ponad pół roku temu" in date_text
                    ):
                ...
                for li in li_tags:
                    for element in li:
                        if (
                            element.name == "span"
                            and element.text.strip() == "Zaktualizowane"
                        ):
                            for sibling in li:
                ...
                if time_posted is None:
                    for li in li_tags:
                        for element in li:
                            if (
                                element.name == "span"
                                and element.text.strip() == "Dodane"
                            ):
                                for sibling in li:
                                    if sibling.name == "div" and sibling.get(
                                        "class"
                                    ) == ["parameters__value"]:
                                        date_text = sibling.text.strip()
                                        time_posted = extract_time(date_text)
            ...
            for phone_number in phone_numbers:
                ...
                print(
                    f'{datetime.datetime.now().strftime("%H:%M:%S")} - {data} | (Поток -> {thread_id})'
                )
...
def main_thread(url, thread_id, lock):
    ...
    while True:
        next_page_link = get_info(
            url=url,
            filename=filename,
            links_counter_file=links_counter_file,
            thread_id=thread_id,
            lock=lock,
        )
