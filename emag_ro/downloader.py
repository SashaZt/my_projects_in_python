...
class Downloader:
    ...
    def get_all_page_json(self, url):
        ...
        if response.status_code == 200:
            ...
            self.dowload_json(response.json())
            # json_response = response.json()
            # job_status = json_response.get("statusUrl")
            ...
            #     # Обновление статуса: отправка нового запроса (пример)
            #     # Предполагается, что `status_url` передаётся в JSON-ответе
            #     status_url = json_response.get("statusUrl")
            #     if status_url:
            #         response = requests.get(status_url)
            #     else:
            #         logger.error("Не удалось получить URL для проверки статуса.")
            #         return
    ...
    def dowload_json(self, json_data):
        ...
        for _ in range(max_retries):
            if response.status_code == 200:
                ...
                logger.info(json_response_new)
                job_status = json_response_new.get("status")
                logger.info(job_status)
                if job_status == "finished":
                    name_file = json_response_new.get("id")
                    json_file = f"{name_file}.json"
                    extracted_body = json_response_new.get("response", {}).get("body")
