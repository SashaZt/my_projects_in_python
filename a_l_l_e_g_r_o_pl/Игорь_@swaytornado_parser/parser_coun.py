...
def parsing_json():
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # }
        futures = {
            executor.submit(parse_single_html_json, file_html): file_html
            for file_html in all_files
        }
...
def list_html():
    ...
    """
    Возвращает список всех файлов из всех папок внутри заданной директории.

    Args:
        html_files_directory (Path): Путь к директории с вложенными папками.

    Returns:
        List[Path]: Список всех найденных файлов.
    """
    all_files = []

    # Проверяем, существует ли директория
    if not html_files.exists() or not html_files.is_dir():
        raise FileNotFoundError(
            f"Директория {html_files} не найдена или не является папкой."
        )

    # Итерация по всем папкам внутри html_files_directory
    for folder in html_files.iterdir():
...
def extract_dimensions(soup):
    ...
    width = None
    height = None
    parametry = extract_params(soup)
    # Проверка наличия параметров и присвоение значений переменным
    if "Waga" in parametry or "Waga produktu z opakowaniem jednostkowym" in parametry:
...
def parse_foto_01(soup):
    ...
    if "imagesrcset" in link_tag.attrs:
        imagesrcset = link_tag["imagesrcset"].split(",")[0]  # Берем первое изображение
...
def pares_sellername(soup):
    ...
    json_data = extract_seller_json(soup)
...
def pares_sellerid(soup):
    ...
    """
    # Извлекаем JSON из dataLayer
    json_data = extract_datalayer_json(soup)
...
def parse_other_product_offers(soup):
    """
    ...
    """
    json_data = extract_price_json(soup)
    try:
...
def pares_iditem(soup):
    ...
    if json_data:
        ...
        sellername = json_data[0].get("idItem") if isinstance(json_data, list) else None
        return sellername

    return None


def pares_productid(soup):
    """
    ...
    json_data = extract_breadcrumbs_json(soup)
...
def pares_reviews_rating(soup):
    ...
    """
    # Извлекаем JSON из <script> с sourceType:product
    json_data = extract_product_json(soup)
    try:
        if isinstance(json_data, dict):
...
def parse_photos(soup):
    ...
    json_data = extract_photo_json(soup)
    image_urls = []
...
def extract_params(soup):
    json_data = extract_parametr_json(soup)
...
def extract_images(soup):
    json_data = extract_images_json(soup)
...
def extract_last_three_urls(soup):
    """
...
def extract_datalayer_json(soup):
    """
...
def extract_parametr_json(soup):
    """
    Извлекает JSON из всех <script type="application/json"> с началом '{"price":{"formattedPric'.
    Для productid
    :param soup: Объект BeautifulSoup для HTML-страницы
...
def extract_breadcrumbs_json(soup):
    """
    Извлекает JSON из всех <script type="application/json"> с началом '{"price":{"formattedPric'.
    Для productid
    :param soup: Объект BeautifulSoup для HTML-страницы
...
def extract_description_json(soup):
    """
    Извлекает JSON из всех <script type="application/json"> с началом '{"price":{"formattedPric'.
    Для productid
    :param soup: Объект BeautifulSoup для HTML-страницы
...
def parse_sales_all_product(soup):
...
def parse_number_of_reviews(soup):
    """Извлекает средний рейтинг из JSON-данных на странице."""
    script_tags = soup.find_all("script", type="application/json")
    average_rating = None
...
def get_url_html_csv():
    ...
    logger.info(f"Парсинг завершен, ссылки сохранены в {csv_output_file}")
...
def parse_single_html(file_html):
    ...
    with open(file_html, encoding="utf-8") as file:
        src = file.read()
    soup = BeautifulSoup(src, "lxml")
    weight, length, width, height = extract_dimensions(soup)
    foto_01 = parse_foto_01(soup)
    fotos = parse_photos(soup)
    # logger.info(fotos)
    max_photos = 9  # Максимальное количество полей Фото_X в company_data
    company_data = {
        "Категория": pares_category(soup),
        "ShopID": pares_sellerid(soup),
        "Наш ID": f"{pares_productid(soup)}-{pares_iditem(soup)}",
        "ID_MP": pares_iditem(soup),
        "SO_ID": pares_productid(soup),
        "SO_CNT": parse_other_product_offers(soup),
        "EAN": parse_ean_product(soup),
        "Марка": parse_brand_product(soup),
        "Название товара": parse_name_product(soup),
        "URL": parse_url_product(soup),
        "Дата": get_current_date(),
        "FOTO_1": foto_01.replace("s512", "original"),
        "Цена, ZLT": parse_price_product(soup),
        "Цена, $": "",
        "Sales": parse_sales_product(soup),
        "Sales_all": parse_sales_all_product(soup),
        "All_Sellers": parse_other_product_offers(soup),
        "Вес": weight,
        "Длина": length,
        "Ширина": width,
        "Высота": height,
        "FOTO_0": foto_01.replace("s512", "s360"),
        "Состояние": parse_condition(soup),
        "Остатки на складах": parse_warehouse_balances(soup),
        "Средняя оценка": parse_average_rating(soup),
        "Количество оценок": parse_number_ratings(soup),
        "Количество отзывов": parse_number_of_reviews(soup),
        "Описание": extract_description_texts(soup),
    }
    extract_params(soup)
...
def parse_single_html_json(file_html):
    """Парсит один HTML-файл для извлечения данных о продукте.
    ...
    all_data = {
        ...
        "reviews_count": parse_number_of_reviews(soup),
        "same_offers_id": pares_productid(soup),
        "same_offers_count": parse_other_product_offers(soup),
        "seller_id": pares_sellerid(soup),
        "seller_login": pares_sellername(soup),
        "seller_rating": "",
    ...
    seller_rating = pares_seller_rating(soup)
    all_data["seller_rating"] = seller_rating

    # br = extract_breadcrumbs(soup)
    all_data["category_path"] = extract_breadcrumbs(soup)
    all_data["images"] = extract_images(soup)
    all_data["description"]["sections"] = extract_description(soup)
    # logger.info(br)
    all_data["reviews_rating"] = pares_reviews_rating(soup)
    all_data["reviews"] = pares_reviews(soup)
    all_names_folders_and_file = extract_last_three_urls(soup)
    data_folder = get_current_date()
    ...
    directory = (
        parent_directory / f'{data_folder}-{all_names_folders_and_file["directory"]}'
    )
    ...
    if json_result.exists():
        logger.info(f"Файл {json_result} есть ")
