...
async def save_to_database(database, table, data, event_time):
    ...
    """
    for item in data:
        await database.execute(query=query, values={'price': item[0], 'quantity': item[1], 'event_time': event_time})
...
async def main_order_book_and_trade(database, number_of_seconds):
    ...
    try:
        async with websockets.connect(uri_depth, ping_interval=10) as websocket_depth, \
                   websockets.connect(uri_trade, ping_interval=10) as websocket_trade:
            print("Соединение установлено.")
            task_depth = asyncio.create_task(receive_data(database, websocket_depth, "Depth"))
            task_trade = asyncio.create_task(receive_data(database, websocket_trade, "Trade"))
...
async def run_for_a_while(number_of_seconds):
    ...
    try:
        await asyncio.wait_for(main_order_book_and_trade(database, number_of_seconds), number_of_seconds)
