...
Файл `.dockerignore` помогает исключить ненужные файлы и папки из контекста сборки Docker. Это уменьшает размер образа и ускоряет процесс сборки.
