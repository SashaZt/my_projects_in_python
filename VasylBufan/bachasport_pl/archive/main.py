...
def get_session() -> Optional[requests.Session]:
    ...
    try:
        ...
        login_headers = {
            ...
            "origin": "https://panel.bachasport.pl",  # Исправлен origin
        ...
        if response.status_code == 200:
            ...
            if final_url != login_url and (
                ...
                if not login_form:
                    ...
                    user_panel = soup.find(
                        "div", {"class": ["user-panel", "user-info"]}
                    )
                    logout_link = soup.find(
                        "a",
                        string=lambda s: s
                        and ("wyloguj" in s.lower() or "logout" in s.lower()),
                    )

                    if user_panel or logout_link:
                        logger.info("Найдены элементы авторизованного интерфейса.")

                        # Пробуем перейти на стартовую страницу вместо dashboard
                        start_url = "https://panel.bachasport.pl/start"
                        check_response = session.get(start_url)

                        if check_response.status_code == 200:
                            logger.info(
                                f"Успешный доступ к стартовой странице: {start_url}"
                            )
                            return session
                        else:
                            logger.warning(
                                f"Не удалось получить доступ к стартовой странице: {check_response.status_code}"
                            )
...
def scrape_pagination(
    session: requests.Session, start_url: str, max_pages: int = 10
) -> List[dict]:
    """
...
def main():
    ...
    try:
        ...
        if not urls:
            logger.error(
                "Не удалось получить ссылки для скрапинга. Завершение программы."
            )
            return
