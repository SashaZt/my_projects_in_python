        ...
        if not filenames:
            ...
            for filename in filenames:
                ...
                with open(filename, "r", encoding="utf-8") as f:
                    ...
                    all_data_table_01_data = safe_get(
                        json_data, [0, "list", 2, "filters", 0, "options"], []
                    )
                    all_data_table_02_data = safe_get(
                        json_data,
                        [
                            0,
                            "list",
                    ...
                    paginated_view = safe_get(
                        json_data, [0, "list", 4, "paginated_view"], []
                    )

                    # Использование безопасного извлечения данных для словарей таблиц
                    dict_table_01 = {
                        entry.get("label"): entry.get("total", "")
                        .replace("USD ", "")
                        .replace(".", ",")
                        for entry in all_data_table_01_data
                    }
                    dict_table_02 = {
                        entry.get("label"): entry.get("value")
                        for entry in all_data_table_02_data
                    }
                    dict_table_03 = {}
                    ...
                    for item in paginated_view:
                        entries = safe_get(
                            item,
                            ["horizontal_line_chart", "datasets", 0, "entries"],
                            [],
                        )
            ...
            for v in values:
                ...
                for product_info in tags_to_list:
                    ...
                    matched_in_tags = [
                        word
                        for word in product_words
                        if word in unique_words_search_terms
                    ]
                    unmatched_in_tags = [
                        word
                        for word in product_words
                ...
                matched_in_search_str_title = ", ".join(
                    list(matched_in_search_set_title)
                )
