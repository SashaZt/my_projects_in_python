...
xml_files_directory.mkdir(parents=True, exist_ok=True)
...
class Get_Response:
    ...
    def fetch_xml(self, url):
        ...
        if response.status_code == 200:
            ...
            # Сохраняем файл в директорию xml_files_directory
            with open(sitemap_file_path, "wb") as file:
                file.write(response.content)
            logger.info(f"Файл сохранен по пути: {sitemap_file_path}")
    ...
    def parsing_sitemap(self):
        # self.fetch_xml(self.url_sitemap)
        if not sitemap_file_path.exists():
            logger.error(
                f"Файл sitemap.xml не найден в директории: {xml_files_directory}"
            )
            return []
        ...
        pattern = r"https://home-club\.com\.ua/sitemap-\d+\.xml"
