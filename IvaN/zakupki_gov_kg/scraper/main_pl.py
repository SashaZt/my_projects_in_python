...
def load_json():
    with open("contracts_data.json", "r", encoding="utf-8") as file:
        data = json.load(file)
        contract_numbers = [item["Номер контракта"] for item in data]
...
async def search_and_save_contract(page, contract_number):
    ...
    try:
        ...
        input_selector = 'input[name="form:j_idt75"]'
        await page.wait_for_selector(input_selector, timeout=10000)
        
        # Очищаем поле и вводим номер контракта
        await page.fill(input_selector, "")
        await page.fill(input_selector, contract_number)
        
        # Нажимаем кнопку "Найти"
        search_button = 'button[id="form:j_idt88"]'
        await page.click(search_button)
        
        # Ждем загрузки результатов таблицы
        await page.wait_for_selector('#form\\:table', timeout=15000)
        await asyncio.sleep(1)  # Дополнительная пауза для полной загрузки
        
        # Проверяем, есть ли результаты в таблице
        table_rows = await page.locator('#form\\:table tbody tr').count()
        if table_rows == 0:
            logger.error(f"❌ Контракт {contract_number} не найден")
            return False
        
        
        # Ищем ссылку с номером контракта в таблице
        contract_link = page.locator(f'a:has-text("{contract_number}")')
        
        # Проверяем, существует ли ссылка
        if await contract_link.count() == 0:
            logger.error(f"❌ Ссылка для контракта {contract_number} не найдена")
            return False
        ...
        # await asyncio.sleep(3)  # Дополнительная пауза
...
def check_existing_files(contract_numbers):
    ...
    for contract_number in contract_numbers:
        ...
        if file_path.exists():
            ...
            logger.info(f"📁 Файл {contract_number}.html уже существует")
        else:
...
async def process_contract_thread(thread_id, url, contract_numbers):
    ...
    """
    logger.info(f"🚀 Поток {thread_id}: начинаем обработку {len(contract_numbers)} контрактов")
    ...
    try:
        async with async_playwright() as p:
            ...
            for i, contract_number in enumerate(contract_numbers, 1):
                logger.info(f"🧵 Поток {thread_id}: обрабатываем {i}/{len(contract_numbers)} - {contract_number}")
    ...
    logger.info(f"✅ Поток {thread_id} завершен: успешно {len(successful_contracts)}, ошибок {len(failed_contracts)}")
    return successful_contracts, failed_contracts
...
async def process_contract_list_multithread(url, contract_numbers, num_threads=3):
    ...
    for thread_id, chunk in enumerate(contract_chunks, 1):
        task = asyncio.create_task(
            process_contract_thread(thread_id, url, chunk)
        )
        tasks.append(task)
    ...
    logger.info(f"✅ Успешно загружено: {len(all_successful) - len(existing_contracts)}")
...
async def main(num_threads=5):
    ...
    """
    # URL страницы поиска контрактов
    url = "http://zakupki.gov.kg/popp/view/order/single_source_procurement.xhtml" 
