...
def find_data_registr(soup):
    data_registr_div = soup.find('div', class_='dr_value_subtitle', string='Дата реєстрації')
    if data_registr_div:
...
def find_record_number(soup):
    ""
    record_number_div = soup.find('div', class_='dr_value_subtitle', string='Номер запису')
    if record_number_div:
        return record_number_div.find_next('div', class_='dr_value').text.strip()
    return None

def find_tax_debt(soup):
    debt_div = soup.find('div', class_='dr_value_subtitle', string=lambda text: text and text.startswith('Податковий борг станом на'))
    if debt_div:
        ...
        if dr_value_div:
            ...
            if t_element:
                ...
                status = ''.join([char for char in status if char.isdigit() or char == '.'])
                if status:  # Если что-то нашли, используем это как статус, иначе "Відсутній"
                    status = status
                else:
                    status = "Відсутній"
            return date, status
...
def find_wage_arrears(soup):
    arrears_div = soup.find('div', class_='dr_value_subtitle', string=lambda text: text and text.startswith('Заборгованість по заробітній платі станом на'))
    if arrears_div:
        ...
        if dr_value_div:
            ...
            if t_element:
                ...
                import re
                match = re.search(r'\b(\d+(?:\.\d+)?)\s*(?:грн|грн\.|гривень)\b', status_text)
                if match:
...
def extract_kved_codes(soup):
    kved_block = soup.find('div', string=lambda text: text and 'Види діяльності' in text)
    if not kved_block:
        return None, None
    ...
    if next_dr_column:
        main_kved_div = next_dr_column.find('div', string=lambda text: text and 'Код КВЕД' in text)
        if main_kved_div:
    ...
    additional_codes = []
    for div in next_dr_column.find_all('div', class_='dr_value dr_margin_value', string=lambda text: text and text[0].isdigit()):
        code = div.text.split()[0]  # Предполагаем, что код идет первым в строке
        if code != main_code:  # Чтобы не дублировать основной код, если он совпадает с дополнительным
...
def main():
    ...
    # for html_file in list(html_directory.glob('*.html'))[:20000]:
