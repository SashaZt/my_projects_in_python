...
def adapt_booking_com_data(reservation_data, reservation_id=None):
    ...
    if is_booking_id:
        logger.debug(f"Адаптация данных Booking.com для бронирования {reservation_id or getattr(reservation_data, 'id', 'unknown')}")
        
        # Адаптируем данные клиента
        if hasattr(reservation_data, 'customer') and reservation_data.customer is not None:
        ...
        if not hasattr(reservation_data, 'responsibleUserId') or reservation_data.responsibleUserId is None:
...
async def get_reservations(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve reservations: {str(e)}")
...
async def update_reservation(
    ...
    try:
        ...
        if reservation_id.isdigit():
            ...
            logger.debug(f"Данные Booking.com адаптированы для обновления: {reservation_id}")
        
        result = await ReservationService.update_reservation(db, reservation_id, reservation)
        if not result:
            raise HTTPException(status_code=404, detail=f"Reservation with ID {reservation_id} not found")
        return result
    except HTTPException:
        ...
        raise HTTPException(status_code=500, detail=f"Не удалось обновить бронирование: {str(e)}")
...
async def delete_reservation(
    ...
    try:
        ...
        if not result:
            raise HTTPException(status_code=404, detail=f"Бронирование с ID {reservation_id} не найдено")
        return None
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Ошибка при удалении бронирования: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Не удалось удалить бронирование: {str(e)}")
...
async def create_bulk_reservations(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка при массовом создании бронирований: {str(e)}")
...
async def update_reservation_status(
    ...
    try:
        ...
        result = await ReservationService.update_reservation(db, reservation_id, update_data)
        if not result:
            raise HTTPException(status_code=404, detail=f"Бронирование с ID {reservation_id} не найдено")
        ...
        raise HTTPException(status_code=500, detail=f"Не удалось обновить статус бронирования: {str(e)}")