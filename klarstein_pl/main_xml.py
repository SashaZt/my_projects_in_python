...
def create_offer_element(offers_parent, offer_data):
    ...
    price.text = str(offer_data.get("price", "0"))

    # ИСПРАВЛЕНО: Оптовые цены в структуре prices с quantity из БД
    price_opt = offer_data.get("price_opt")
    quantity = offer_data.get("quantity", 10)  # Берем из БД или по умолчанию 10

    if price_opt and str(price_opt).strip() and float(str(price_opt)) > 0:
        prices = ET.SubElement(offer, "prices")
        price_elem = ET.SubElement(prices, "price")
        value = ET.SubElement(price_elem, "value")
        value.text = str(price_opt)
        quantity_elem = ET.SubElement(price_elem, "quantity")
        quantity_elem.text = str(quantity)  # ИСПРАВЛЕНО: используем quantity из БД
...
def create_xml_from_products_list(products_list, output_file):
    ...
    for product in products_list:
        offer = {
            ...
            "price_opt": str(product.get("price_opt", "0")),
