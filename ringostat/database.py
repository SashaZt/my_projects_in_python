        ...
        async with self.pool.acquire() as connection:
            async with connection.cursor(aiomysql.DictCursor) as cursor:
                try:
                    ...
                    # Создаем запрос с динамическим объединением всех таблиц
                    base_query = """
                    ...
                    for table in contact_tables:
                        table_suffix = table.split('_', 1)[1]  # Получаем суффикс после "contacts_"
                        table_alias = f"{table_suffix[:2]}{alias_counter}"  # Создаем уникальный алиас
                        join_field = f"{table_alias}.contact_id"
                        join_queries.append(f"LEFT JOIN {table} {table_alias} ON c.id = {join_field}")
                        ...
                        for column in columns:
                            ...
                            if column_name != 'contact_id':
                                ...
                                if filters and column_name in filters:
                                    where_clauses.append(f"{table_alias}.{column_name} = %s")
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    logger.info(f"Данные успешно добавлены в таблицу calls: {call_data}")
            return True
        except Exception as e:
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    await connection.commit()
                    logger.info(f"Данные успешно добавлены в таблицу contacts_phone_numbers: {phone_data}")
            return True
        except Exception as e:
            logger.error(f"Ошибка при добавлении данных в таблицу contacts_phone_numbers: {e}")
            return False
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    await connection.commit()
                    logger.info(f"Данные успешно добавлены в таблицу contacts_emails: {email_data}")
            return True
        except Exception as e:
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    await connection.commit()
                    logger.info(f"Данные успешно добавлены в таблицу contacts_bank_accounts: {bank_account_data}")
            return True
        except Exception as e:
            logger.error(f"Ошибка при добавлении данных в таблицу contacts_bank_accounts: {e}")
            return False
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    await connection.commit()
                    logger.info(f"Данные успешно добавлены в таблицу contact_managers: {manager_data}")
            return True
        except Exception as e:
            logger.error(f"Ошибка при добавлении данных в таблицу contact_managers: {e}")
            return False
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    await connection.commit()
                    logger.info(f"Данные успешно добавлены в таблицу contact_status: {status_data}")
            return True
        except Exception as e:
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    await connection.commit()
                    logger.info(f"Данные успешно добавлены в таблицу contacts_interaction_history: {interaction_data}")
            return True
        except Exception as e:
            logger.error(f"Ошибка при добавлении данных в таблицу contacts_interaction_history: {e}")
            return False
        ...
        try:
            async with self.pool.acquire() as connection:
                async with connection.cursor() as cursor:
                    ...
                    await connection.commit()
                    logger.info(f"Данные успешно добавлены в таблицу contacts_addresses: {address_data}")
            return True
        except Exception as e:
            logger.error(f"Ошибка при добавлении данных в таблицу contacts_addresses: {e}")
            return False
