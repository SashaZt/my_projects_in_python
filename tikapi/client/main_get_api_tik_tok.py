...
def user_live_analytics():
    ...
    for user in users:
        ...
        user_data_file = (
            temp_directory / f"user_live_analytics_{timestamp}_{tik_tok_id}.json"
        )
        ...
        try:
            ...
            while attempt < max_attempts and not success:
                try:
                    ...
                    if hasattr(response, "status_code") and response.status_code == 200:
                        ...
                        attempt += 1
                        logger.warning(
                            f"Attempt {attempt}/{max_attempts}: Got status code {getattr(response, 'status_code', 'unknown')}. Retrying in 5 seconds..."
                        )
                        time.sleep(5)
                except Exception as e:
                    attempt += 1
                    logger.warning(
                        f"Attempt {attempt}/{max_attempts} failed with error: {e}. Retrying in 5 seconds..."
                    )
                    time.sleep(5)
            ...
            if not success:
                logger.error(
                    f"Failed to get analytics for user {tik_tok_id} after {max_attempts} attempts"
                )
                continue
...
def user_live_list():
    ...
    for user in users:
        ...
        account_user = api_key.user(accountKey=account_key)
        user_live_list_file = (
            temp_directory / f"user_live_list_{timestamp}_{tik_tok_id}.json"
        )
            ...
            try:
                ...
                if hasattr(response, "status_code") and response.status_code == 200:
                    ...
                    with open(user_live_list_file, "w", encoding="utf-8") as json_file:
                        json.dump(
                            response.json(), json_file, ensure_ascii=False, indent=4
                        )
                else:
                    # Если статус не 200, ждем и повторяем
                    attempt += 1
                ...
                attempt += 1
                logger.error(
                    f"Validation error (attempt {attempt}/{max_attempts}): {e}"
                )
                if hasattr(e, "field"):
                    logger.error(f"Field with error: {e.field}")
                time.sleep(5)
                ...
                attempt += 1
                logger.error(
                    f"Unexpected error (attempt {attempt}/{max_attempts}): {e}"
                )
                time.sleep(5)
        ...
        if not success:
            logger.error(
                f"Failed to get live list for user {tik_tok_id} after {max_attempts} attempts"
            )
...
def user_info():
    ...
    for user in users:
        ...
        while attempt < max_attempts and not success:
            try:
                ...
                if hasattr(response, "status_code") and response.status_code == 200:
                    ...
                    user_info_file = (
                        temp_directory / f"user_info_{timestamp}_{account_key}.json"
                    )
                ...
                attempt += 1
                logger.error(
                    f"Response error {account_key} (attempt {attempt}/{max_attempts}): {e}"
                )
                if hasattr(e, "response") and hasattr(e.response, "status_code"):
                    ...
                    if status_code == 401:
                        logger.error(
                            f"ТРЕБУЕТСЯ ПРОВЕРКА АККАУНТА! ОШИБКА АВТОРИЗАЦИИ 401 для account_key: {account_key}"
                        )
                        ...
                        try:
                            ...
                            if auth_error_file.exists():
                                with open(
                                    auth_error_file, "r", encoding="utf-8"
                                ) as error_file:
                            ...
                            with open(
                                auth_error_file, "w", encoding="utf-8"
                            ) as error_file:
                                json.dump(
                                    auth_errors,
                                    error_file,
                            ...
                            logger.error(
                                f"Не удалось записать информацию об ошибке авторизации: {save_err}"
                            )
        ...
        if not success:
            logger.error(
                f"Failed to get user info for account {account_key} after {max_attempts} attempts"
            )
