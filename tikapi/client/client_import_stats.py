...
async def import_user_stats(file_path="user_info.json"):
    ...
    try:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            ...
            logger.info(
                f"Проверка статистики для пользователя: {user['unique_id']} за дату: {today}"
            )

            # Получаем последнюю статистику
            try:
                stats_check = await client.get(
                    f"{API_BASE_URL}/stats/user-stats/{user['id']}"
                )
                ...
                if stats_check.status_code == 200:
                    ...
                    if stats_list:
                        ...
                        for stat in stats_list:
                            if "timestamp" in stat and stat["timestamp"].startswith(
                                today
                            ):
                                today_stats = stat
                                break
                    ...
                    if today_stats:
                        ...
                        logger.info(
                            f"Обновление существующей статистики пользователя: {user['unique_id']}"
                        )
                        ...
                        if stats_response.status_code == 200:
                            ...
                            logger.error(
                                f"Ошибка при обновлении статистики: {stats_response.status_code}"
                            )
                    else:
                        # Если нет статистики за сегодня, создаем новую
                        logger.info(
                            f"Добавление новой статистики для пользователя: {user['unique_id']}"
                        )
                        stats_response = await client.post(
                            f"{API_BASE_URL}/stats/user-stats", json=stats_payload
                        )
                        ...
                        if stats_response.status_code == 200:
                            ...
                            logger.error(
                                f"Ошибка при добавлении статистики: {stats_response.status_code}"
                            )
                else:
                    # Создаем статистику, если GET запрос не удался
                    logger.info(
                        f"Добавление первичной статистики для пользователя: {user['unique_id']}"
                    )
                    stats_response = await client.post(
                        f"{API_BASE_URL}/stats/user-stats", json=stats_payload
                    )
            ...
            if user["nickname"] != user_data.get("nickname"):
                logger.info(
                    f"Обнаружено изменение никнейма для пользователя {user['unique_id']}"
                )
                ...
                history_response = await client.post(
                    f"{API_BASE_URL}/stats/nickname-history",
                    json=nickname_history_payload,
                )
                ...
                if history_response.status_code == 200:
                    ...
                    logger.error(
                        f"Ошибка при сохранении истории никнейма: {history_response.status_code}"
                    )

                # Обновляем пользователя с новым никнеймом
                update_payload = {"nickname": user_data.get("nickname")}

                update_response = await client.put(
                    f"{API_BASE_URL}/users/{tiktok_id}", json=update_payload
                )

                if update_response.status_code == 200:
                    logger.info(
                        f"Никнейм пользователя успешно обновлен на: {user_data.get('nickname')}"
                    )
                else:
                    logger.error(
                        f"Ошибка при обновлении никнейма: {update_response.status_code}"
                    )

            # Проверяем и обновляем unique_id если он изменился
            if user["unique_id"] != user_data.get("uniqueId"):
                logger.info(
                    f"Обнаружено изменение unique_id для пользователя {user['nickname']}"
                )
                ...
                history_response = await client.post(
                    f"{API_BASE_URL}/stats/uniqueid-history",
                    json=uniqueid_history_payload,
                )
                ...
                if history_response.status_code == 200:
                    ...
                    logger.error(
                        f"Ошибка при сохранении истории unique_id: {history_response.status_code}"
                    )

                # Обновляем пользователя с новым unique_id
                update_payload = {"unique_id": user_data.get("uniqueId")}
                ...
                if update_response.status_code == 200:
                    logger.info(
                        f"Unique_id пользователя успешно обновлен на: {user_data.get('uniqueId')}"
                    )
                else:
                    logger.error(
                        f"Ошибка при обновлении unique_id: {update_response.status_code}"
                    )
