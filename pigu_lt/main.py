...
def get_product_id(soup):
    ...
    row = soup.find("tr", string=lambda t: t and "Prekės ID:" in t)
    if row:
...
def get_price(soup):
    ...
    price_container = soup.select_one(
        ".c-price.h-price--xx-large, .c-price.h-price--xx-large.h-price--new, .c-price.h-price--xx-large.h-price"
    )
    price = None
    currency = None
    if price_container:
        ...
        fractional_price = (
            price_container.select_one("sup").text.strip()
            if price_container.select_one("sup")
            else "00"
        )
        # Извлекаем валюту
        currency = (
            price_container.select_one("small").text.strip()
            if price_container.select_one("small")
            else ""
        )
        # Форматируем цену
...
def get_reviews_and_ratings(soup):
    ...
    if reviews_section:
        ...
        reviews_count = (
            reviews_section.select_one('a[href*="#reviews"] span')
            .text.strip()
            .replace("(", "")
            .replace(")", "")
        )
...
def get_all_photos(soup):
    ...
    for photo in photo_elements:
        ...
        photos.append(
            {"original": original_url, "thumbnail": thumbnail_url, "alt": alt_text}
        )
    return photos
...
def get_description(soup):
    description_element = soup.select_one(
        "#product-description .c-product__description-container"
    )
...
def get_reviews(soup):
    ...
    for review in review_elements:
        ...
        text = (
            review.select_one(".c-review__content p").text.strip()
            if review.select_one(".c-review__content p")
            else ""
        )
        # Считаем количество активных звезд для определения рейтинга
        score = len(
            review.select(".c-rating.s-is-active .c-icon--star-full.s-is-active")
        )
        # Извлекаем дату отзыва
        date = (
            review.select_one(".c-review__date").text.strip()
            if review.select_one(".c-review__date")
            else ""
        )
        # Добавляем данные отзыва в список
        reviews.append({"text": text, "score": score, "date": date})
...
def get_seller_offers(soup):
    ...
    for seller in seller_elements:
        ...
        reviews_count = (
            int(re.search(r"\d+", reviews_text.text).group()) if reviews_text else 0
        )
        ...
        if price_element:
            ...
            euros = euros.replace(" ", "")  # Удаляем пробелы из числа
            cents = (
                price_element.select_one("sup").text.strip()
                if price_element.select_one("sup")
                else "00"
            )
        ...
        # Валюта
        currency = (
            price_element.select_one("small").text.strip()
            if price_element and price_element.select_one("small")
            else "€"
        )
