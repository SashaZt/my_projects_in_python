...
def extract_product_data(data):
    ...
    title = data.find("input", attrs={"name": "productTitle"})
    title = title.get("value")
    price = data.find("input", attrs={"name": "priceValue"})
    if price:
    ...
    article_number = data.find("input", attrs={"name": "items[0.base][asin]"})
    if article_number:
    ...
    availability_tag = data.find("div", attrs={"id": "availability"})
    logger.info(f"availability_tag: {availability_tag}")

    if availability_tag:
        availability = availability_tag.find(
