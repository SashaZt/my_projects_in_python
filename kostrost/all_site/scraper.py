...
class ExcelSheetScraper:
    def __init__(
        ...
        # Добавим атрибуты для циклической проверки
        self.check_interval = 30  # секунд между полными циклами проверок
        self.job_check_interval = 2  # секунд между проверками одного задания
        self.max_check_attempts = (
            10  # максимальное количество попыток проверки одного задания
        )
    ...
    def submit_jobs(self, sheet_urls):
        ...
        for sheet_name, urls in sheet_urls.items():
            ...
            for url in urls:
                ...
                existing_job = self._find_existing_job(url)
                if existing_job:
                    logger.info(f"Для URL {url} уже есть задание, пропускаем")
                    continue
    ...
    async def check_job_with_retries(self, job_info):
        ...
        for attempt in range(self.max_check_attempts):
            status, html_content = self.check_existing_job_status(job_info)
            ...
            if status == "finished" and html_content:
                ...
                if attempt < self.max_check_attempts - 1:
                    logger.info(
                        f"Ожидание {self.job_check_interval} секунд перед следующей попыткой..."
                    )
    ...
    async def continuous_check_jobs(self):
        """
        Непрерывно проверяет все задания в цикле
        """
        while True:
            active_jobs = []

            # Собираем все текущие задания
            for json_file in self.json_dir.glob("*.json"):
                try:
                    with open(json_file, "r", encoding="utf-8") as f:
                        job_info = json.load(f)

                    # Пропускаем завершенные задания
                    if (
                        job_info.get("status") == "finished"
                        or job_info.get("status") == "failed"
                    ):
                        continue

                    active_jobs.append((json_file, job_info))
                except Exception as e:
                    logger.error(
                        f"Ошибка при чтении файла задания {json_file}: {str(e)}"
                    )

            if not active_jobs:
                logger.info("Нет активных заданий для проверки")

                # Проверяем, есть ли новые URL для скрапинга
                sheet_urls = self.extract_urls_from_excel()
                if self.submit_jobs(sheet_urls):
                    logger.info("Новые задания отправлены, продолжаем цикл проверок")
                else:
                    logger.info("Новых URL для обработки нет, ожидание...")
                    await asyncio.sleep(self.check_interval)
                continue

            logger.info(f"Проверка {len(active_jobs)} активных заданий...")
            completed_count = 0

            # Проверяем каждое задание
            for json_file, job_info in active_jobs:
                job_id = job_info.get("id")
                url = job_info.get("url")
                html_file_path = job_info.get("html_file")
                ...
                if status == "finished" and html_content:
                    ...
                    try:
                        ...
                        # Обновляем статус задания или удаляем его
                        try:
                            json_file.unlink()  # Удаляем файл задания
                    ...
                    try:
                        ...
                        logger.error(f"Ошибка при обновлении статуса задания: {str(e)}")

                # Добавляем небольшую задержку между проверками заданий
                await asyncio.sleep(0.5)

            logger.info(
                f"Цикл проверок завершен. Обработано заданий: {completed_count}/{len(active_jobs)}"
            )

            # Проверяем, есть ли новые URL для скрапинга
            if len(active_jobs) == 0 or completed_count == len(active_jobs):
                sheet_urls = self.extract_urls_from_excel()
                if self.submit_jobs(sheet_urls):
                    logger.info("Новые задания отправлены")

            # Пауза перед следующим циклом проверок
            logger.info(
                f"Ожидание {self.check_interval} секунд перед следующим циклом проверок..."
            )
            await asyncio.sleep(self.check_interval)

    async def main(self):
        """Основная функция для запуска процесса скрапинга"""
        logger.info("Запуск скрапера в режиме непрерывной проверки")

        # Проверяем, есть ли URL для скрапинга
        sheet_urls = self.extract_urls_from_excel()
        initial_jobs_submitted = self.submit_jobs(sheet_urls)

        if initial_jobs_submitted:
            logger.info("Начальные задания отправлены")
        else:
            logger.info("Нет новых URL для обработки или все файлы уже существуют")

        # Запускаем непрерывную проверку заданий
        await self.continuous_check_jobs()
