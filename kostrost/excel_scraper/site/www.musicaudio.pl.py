...
def extract_product_data(data):
    ...
    title = data.find("input", attrs={"name": "products_name"})
    title = title.get("value")
    price = data.find("input", attrs={"name": "products_price"})
    if price:
        price = price.get("value")
    else:
        logger.warning("Тег с ценой не найден")
        price = None
    article_number = data.find("input", attrs={"name": "products_id"})
    if article_number:
